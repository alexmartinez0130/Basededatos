# Diccionario de Datos de la Base de Datos Gestión de Empresa

## 1. Información General

| Elemento | Valor |
| :--- | :--- |
| Proyecto | Sistema de Gestión Empresarial |
| Versión | 1.0 |
| Fecha | Junio 2026 |
| Elaboró | Alexis Daniel Martinez Mendoza |
| SGBD | SQL Server |

---

# 2. Descripción del Sistema de Base de Datos

El sistema administra la información de los empleados, departamentos, proyectos y las ubicaciones donde opera la empresa.

Permite controlar:

- Empleados
- Departamentos
- Proyectos
- Ubicaciones de Departamentos

También administra la participación de los empleados en los proyectos.

---

# 3. Catálogo de Restricciones Utilizadas

| Código | Significado |
| :--- | :--- |
| PK | Primary Key |
| FK | Foreign Key |
| NN | Not Null |
| UQ | Unique |
| AI | Auto Increment |
| CK | Check |
| DF | Default |

---

# 4. Diccionario de Datos

## Tabla: Empleado

*Descripción:* Almacena la información de los empleados.

| Campo | Tipo | Longitud | Restricciones | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| NSS | INT | - | PK, NN | Número de Seguro Social del empleado |
| Nombre | VARCHAR | 40 | NN | Nombre del empleado |
| Apellido | VARCHAR | 40 | NN | Apellido del empleado |
| Salario | DECIMAL | 10,2 | NN | Salario del empleado |
| NSSSupervisor | INT | - | FK | Supervisor inmediato |
| NumDepartamento | INT | - | FK, NN | Departamento al que pertenece |

---

## Tabla: Departamento

*Descripción:* Almacena la información de los departamentos.

| Campo | Tipo | Longitud | Restricciones | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| NumDepartamento | INT | - | PK, NN | Identificador del departamento |
| NombreDepartamento | VARCHAR | 50 | NN | Nombre del departamento |
| NSSGerente | INT | - | FK | Empleado gerente |
| FechaInicio | DATE | - | NN | Fecha de inicio del gerente |

---

## Tabla: Proyecto

*Descripción:* Almacena los proyectos desarrollados por la empresa.

| Campo | Tipo | Longitud | Restricciones | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| NumProyecto | INT | - | PK, NN | Identificador del proyecto |
| NombreProyecto | VARCHAR | 60 | NN | Nombre del proyecto |
| Lugar | VARCHAR | 50 | NN | Lugar donde se desarrolla |
| NumDepartamento | INT | - | FK, NN | Departamento responsable |

---

## Tabla: UbicacionDepartamento

*Descripción:* Almacena las ubicaciones físicas de los departamentos.

| Campo | Tipo | Longitud | Restricciones | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| NumDepartamento | INT | - | PK, FK | Departamento |
| Ubicacion | VARCHAR | 50 | PK, NN | Ubicación del departamento |

---

## Tabla: TrabajaEn

*Descripción:* Registra las horas que trabaja cada empleado en los proyectos.

| Campo | Tipo | Longitud | Restricciones | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| NSS | INT | - | PK, FK | Empleado |
| NumProyecto | INT | - | PK, FK | Proyecto |
| Horas | DECIMAL | 4,1 | NN | Horas trabajadas |

---

# 5. Relaciones

| Relación | Cardinalidad | Descripción |
| :--- | :---: | :--- |
| Departamento → Empleado | 1:N | Un departamento tiene muchos empleados. |
| Departamento → Proyecto | 1:N | Un departamento administra varios proyectos. |
| Departamento → UbicaciónDepartamento | 1:N | Un departamento puede tener varias ubicaciones. |
| Empleado → TrabajaEn | 1:N | Un empleado puede trabajar en varios proyectos. |
| Proyecto → TrabajaEn | 1:N | Un proyecto puede tener varios empleados. |
| Empleado → Empleado | 1:N | Un supervisor puede supervisar varios empleados. |

---

# 6. Matriz de Claves Foráneas

| Tabla | Campo FK | Referencia |
| :--- | :--- | :--- |
| Empleado | NumDepartamento | Departamento (NumDepartamento) |
| Empleado | NSSSupervisor | Empleado (NSS) |
| Departamento | NSSGerente | Empleado (NSS) |
| Proyecto | NumDepartamento | Departamento (NumDepartamento) |
| UbicacionDepartamento | NumDepartamento | Departamento (NumDepartamento) |
| TrabajaEn | NSS | Empleado (NSS) |
| TrabajaEn | NumProyecto | Proyecto (NumProyecto) |

---

# 7. Integridad Referencial

| Regla | Descripción |
| :--- | :--- |
| IR-01 | Todo empleado debe pertenecer a un departamento existente. |
| IR-02 | Todo proyecto debe pertenecer a un departamento existente. |
| IR-03 | No puede registrarse una ubicación para un departamento inexistente. |
| IR-04 | No puede registrarse un empleado en un proyecto inexistente. |
| IR-05 | El gerente debe existir como empleado. |

---

# 8. Reglas del Negocio

| Código | Regla |
| :--- | :--- |
| RN-01 | Un departamento puede tener varios empleados. |
| RN-02 | Un empleado pertenece únicamente a un departamento. |
| RN-03 | Un proyecto pertenece a un solo departamento. |
| RN-04 | Un empleado puede participar en varios proyectos. |
| RN-05 | Cada proyecto puede tener varios empleados asignados. |
| RN-06 | Un departamento puede tener varias ubicaciones físicas. |

---

# 9. Diagrama Relacional

Departamento (1) ─────────< Empleado

Departamento (1) ─────────< Proyecto

Departamento (1) ─────────< UbicacionDepartamento

Empleado (N) >────────< (N) Proyecto
          (Tabla TrabajaEn)

Empleado (1) ─────────< Empleado